...
void Rigidbody::Update()
{
	...
	*pos = *pos + (vel * Engine::GetDT());
	...
	if (lastRot != *rot)
	{
		boundingRect.lowerLeftVertex = Math::RotatePoint(boundingRect.lowerLeftVertex, Vector3(0), (*rot - lastRot)/2);
		boundingRect.lowerRightVertex = Math::RotatePoint(boundingRect.lowerRightVertex, Vector3(0), (*rot - lastRot)/2);
		boundingRect.upperLeftVertex = Math::RotatePoint(boundingRect.upperLeftVertex, Vector3(0), (*rot - lastRot)/2);
		boundingRect.upperRightVertex = Math::RotatePoint(boundingRect.upperRightVertex, Vector3(0), (*rot - lastRot)/2);
		lastRot = *rot;
	}
}
